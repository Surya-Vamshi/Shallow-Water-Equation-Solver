...
# 2D Shallow Water Equation code for Time-dependent Source
# ------------------------------
def SWE_solver_time_dependent(eta0, qx_init, qy_init, h, g, T_end, dx, dy, dt, dbdt_grid, sigma_t, case, plot_min, plot_max):
    """
    Solving 2D Shallow water equation using Time-dependent Source method.
    ...
    dt : float
        Time step.
    dbdt_grid : np.ndarray
        Source term.
    sigma_t : float
        Characterized duration of source.
    case : basestring
        Name of the case
    plot_min : float
        Limit for plot.
    ...
    # import
    from update_eta_time_dependent import update_eta_time_dependent
    ...
    t = 0
    output = 0
    result = np.empty((nx, int(T_end + 1)))
    ...
    for n in range(nt):
        ...
        # Update Eta field
        eta = update_eta_time_dependent(eta, qx, qy, dx, dy, t, dt, nx, ny, dbdt_grid, sigma_t)
    ...
    plt.colorbar()
    plt.savefig("../Output/Time_dependent/Result_" + case, dpi=125)
    # plt.show()
